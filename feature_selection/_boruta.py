...
from ..ensemble._lightgbm import isa_lightgbm_model


class BaseBorutaSelection(BaseEstimator, SelectorMixin):
    """
    ...
    - Scikit-learn like interface:
        Use BaseBorutaSelection just like any other scikit learner: fit, fit_transform and
    ...
    - Ranking of features:
        After fitting BaseBorutaSelection it provides the user with ranking of features.
    ...
    from boruta import BaseBorutaSelection
    
    # load X and y
    # NOTE BaseBorutaSelection accepts numpy arrays only, hence the .values attribute
    >>> from orca_ml.datasets import load_uci_credit
    >>> X, y = load_uci_credit(return_X_y=True, as_frame=True)
    >>> from orca_ml.ensemble import RandomForestClassifier
    >>> rf = RandomForestClassifier(n_jobs=-1, class_weight= balanced", max_depth=5, random_state=100)l
    >>> from orca_ml.feature_selection import BorutaSelection
    ...
    feat_selector = BaseBorutaSelection(rf, n_estimators='auto', verbose=2, random_state=1)
    ...
    def __init__(self, *, n_estimators=1000, percentile=100, alpha=0.05,
    ...
    def _validate_keywords(self):
        if self.percentile <= 0 or self.percentile > 100:
            raise ValueError("The percentile should be between O and 100.")
        if self.alpha <= 0 or self.alpha > 1:
            raise ValueError("Alpha should be between 0 and 1.")
        if not hasattr(self.estimator, "n_estimators"):
            raise ValueError("Estimator should be an ensemble tree.")
    ...
    def _fit(self, X, y):
        ...
        X, y = check_X_y(X, y, dtype="numeric", force_all_finite=True, ensure_2d=True, y_numeric=False)
        ...
        while np.any(dec_reg == 0) and _iter < self.max_iter:
            ...
            if isa_lightgbm_model(self.estimator):
            ...
            imp_sha_max = np.percentile(cur_imp[1], self.percentile)
    ...
    def _get_imp(self, X, y):
        ...
        try:
            ...
            raise ValueError('Only methods with feature_importance_ attribute '
                            'are currently supported in BaseBorutaSelection.')
        return imp
    ...
    def _print_results(self, dec_reg, _iter, flag):
        ...
        # Boruta finished running and tentatives have been filtered
            ...
            content = map(str, [n_iter, n_confirmed, n_tentative, n_rejected])
            result = '\n'.join([x[0] + '\t' + x[1] for x in zip(cols, content)])
            output = "\n\nBaseBorutaSelection finished running.\n\n" + result
        print(output)


class BorutaSelection(BaseBorutaSelection, MetaEstimatorMixin):
    @_deprecate_positional_args
    def __init__(self, estimator, *, n_estimators=100, percentile=100, alpha=0.05,
                 two_step=True, max_iter=100, weak=False, random_state=None, verbose=0):
        super(BorutaSelection, self).__init__(n_estimators=n_estimators, percentile=percentile, alpha=alpha,
                                              two_step=two_step, max_iter=max_iter, weak=weak, random_state=random_state, verbose=verbose)
...
def _fdrcorrection(pvals, alpha=0.5):
    """
    Benjamini/Hochberg p-value correction for false discovery rate, from statsmodels package. Include here for decoupling dependency on statsmodels.
    ...
    # reorder p-values and rejection mask to original order of pvals
    # pvals_corrected_ = np.empty_like(pvals_corrected)
    # pvals corrected [pvals sortind] = pvals corrected
    reject_ = np.empty_like(reject)
    reject [pvals_sortind] = reject
    # return reject_, pvals_corrected_
    return reject_


def _nanrankdata(X, axis=1):
    """Replace bottleneck's nanrankdata with scipy and numpy alternative.
