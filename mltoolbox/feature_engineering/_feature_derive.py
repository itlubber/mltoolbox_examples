...
class ExprDerive():
    def __new__(cls, derivings, backend=0):
        if backend in ('NumExprDerive', 0):
            return NumExprDerive(derivings)
        elif backend in ('Aviator', 1):
            from ._aviator_expr_derive import AviExprDerive
            return AviExprDerive(derivings)
        else:
            raise ValueError('backend only support 0,1,"NumExprDerive" or "Aviator"')
...
class NumExprDerive(BaseEstimator, TransformerMixin):
    ...
    >>> import numpy as np
    >>> import pandas as pd
    >>> from mltoolbox.feature_engineering._feature_derive import NumExprDerive
    ...
    def _transform_frame(self, X):
        ...
        context = _get_context(X, feature_names=feature_names)
    ...
    def _transform_ndarray(self, X):
        X = self._validate_data(X, dtype="numeric", ensure_2d=True, force_all_finite=False)
        context = _get_context(X, feature_names=None)
        n_derived = len(self.derivings)
        X_derived = np.empty((X.shape[0], n_derived), dtype=np.float64)
        derived_names = []
        for i, (name, expr) in enumerate(self.derivings):
