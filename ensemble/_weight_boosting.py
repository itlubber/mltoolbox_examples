...
try:
    from sklearn.ensemble.weight_boosting import BaseWeightBoosting, AdaBoostClassifier, AdaBoostRegressor
except ImportError:
    from sklearn.ensemble._weight_boosting import BaseWeightBoosting, AdaBoostClassifier, AdaBoostRegressor
...
class AdaCost(BaseWeightBoosting, ClassifierMixin):
    ...
    .. [3] https://github.com/joelprince25/adacost/blob/master/adacost/adacost.py
    """
    ...
    def cost_table_calc(self, labels, cost_matrix):
        ...
        """
        table = np.vstack([np.array([labels[i], labels[j], value]) for (i, j), value in np.ndenumerate(cost_matrix)])
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if self.cost_matrix is None:
            ...
            self.cost_matrix = np.ones((n_classes, n_classes))
            
        self.cost_table = self.cost_table_calc(labels, self.cost_matrix)
    ...
    def _boost_real(self, iboost, X, y, sample_weight, random_state):
        ...
        estimator_weight = (-1. * self.learning_rate
                                * (((n_classes - 1.) / n_classes) * xlogy(y_coding, y_predict_proba).sum(axis=1)))
    ...
    def staged_decision_function(self, X):
        ...
        Parameters
        -------
        X : farray-like, sparse matrix} of shape = [n_samples, n_features]
            The training input samples. Sparse matrix can be CSC, CSR, COO,
            DOK, or LIL. DOK and LIL are converted to CSR.
...
class AdaBoostClassifierParameterProxy(ModelParameterProxy):
    ...
    def _make_estimator(self):
        estimator = AdaBoostClassifier(base_estimator=self.base_estimator,
...
class AdaBoostRegressorParameterProxy(ModelParameterProxy):
    ...
    def _make_estimator(self):
        estimator = AdaBoostRegressor(base_estimator=self.base_estimator,
                                        n_estimators=self.n_estimators,
                                        learning_rate=self.learning_rate,
                                        loss=self.loss,
                                        random_state=self.random_state)
