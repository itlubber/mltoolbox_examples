...
class BaseDiscretizer(TransformerMixin, BaseEstimator):
    ...
    See Also
    ----------
    orca_ml.preprocessing.Binarizer : Class used to bin values as `0` or `1` based on a parameter `threshold`
    ...
    (e.g., :class:`orca_ml.feature_selection.VarianceThreshold`).

    Examples
    ----------
    >>> from orca_ml.preprocessing import DecisionTreeDiscretizer
    >>>X = [[-2,1,-4,-1],
            [-1, 2, -3, -0.5],
            [0,  3, -2,  0.5],
            [1,  4, -1,    2]]
